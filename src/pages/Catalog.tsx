...
/** Все карточки каталога: квадратная обложка, без растягивания и грубого кропа */
function CatalogSquareCover({ src, alt }: { src: string; alt: string }) {
  return (
...
const cardBodyClass = 'border-t border-border/25 bg-card/90 px-3 py-3.5';
      ...
      <div className="container">
        <div className="mx-auto w-full max-w-6xl">
          ...
          <motion.div
            ...
            onClick={musicBurst}
          >
            {/* Сетка альбомов: 1 колонка на мобилке, 2 на lg; узкий блок по центру */}
            <div className="mx-auto w-full max-w-5xl">
              <div className="grid grid-cols-1 items-start gap-8 sm:gap-8 lg:grid-cols-2 lg:gap-6">
                ...
                <div className="min-w-0">
                  ...
                  <article className="interactive-card mt-5 w-full overflow-hidden p-0 lg:mt-6">
                    <CatalogSquareCover src={catalogMelanoNew.album.coverSrc} alt={catalogMelanoNew.album.title} />
                    <div className={cardBodyClass}>
                      <div className="flex flex-wrap items-start justify-between gap-2">
                        <h2 className="font-display text-lg font-bold leading-tight md:text-xl">{catalogMelanoNew.album.title}</h2>
                      ...
                      <p className="mt-3 text-[10px] font-semibold uppercase tracking-[0.2em] text-muted-foreground">Треклист</p>
                      <ol className="mt-1.5 list-decimal space-y-0.5 pl-4 text-sm leading-snug text-muted-foreground">
                ...
                <div className="min-w-0">
                  ...
                  <article className="interactive-card mt-5 w-full overflow-hidden p-0 lg:mt-6">
                    <CatalogSquareCover src={catalogPrevious.album.coverSrc} alt={catalogPrevious.album.title} />
                    <div className={cardBodyClass}>
                      <h3 className="font-display text-lg font-bold leading-tight md:text-xl">{catalogPrevious.album.title}</h3>
                      <p className="mt-1.5 text-sm text-muted-foreground">Альбом</p>
                    </div>
                  </article>
                </div>
              </div>
            ...
            <div className="mt-12 grid w-full grid-cols-1 gap-6 sm:grid-cols-2">
              {catalogPreviousSingles.map((single, i) => (
                <motion.article
                  ...
                  onClick={musicBurst}
                >
                  <CatalogSquareCover src={single.coverSrc} alt={single.title} />
                  <div className={cardBodyClass}>
                    <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Сингл</p>
                    <h4 className="mt-1 font-display text-lg font-bold leading-tight md:text-xl">{single.title}</h4>
                    <p className="mt-2 text-sm leading-snug text-muted-foreground">{single.description}</p>
                  </div>
                </motion.article>
              ))}
            </div>
            ...
            <div className="mt-10 grid w-full grid-cols-1 gap-6 sm:grid-cols-2">
              {catalogClientWorks.map((work, i) => (
                <motion.article
                  ...
                  onClick={musicBurst}
                >
                  <CatalogSquareCover src={work.coverSrc} alt={work.title} />
                  <div className={cardBodyClass}>
                    <h3 className="font-display text-lg font-bold leading-tight md:text-xl">{work.title}</h3>
                    <p className="mt-2 text-sm leading-snug text-muted-foreground">{work.description}</p>
                  </div>
                </motion.article>
              ))}
            </div>
